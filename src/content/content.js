...
var JobClicker = {
  ...
  scrollAndFindCard: async function(jobLink, positionName, companyName, fastMode) {
    ...
    var scrollMs = fastMode ? 450 : 800;
    var maxRounds = fastMode ? 6 : 12;
  ...
  clickImmediateChat: async function(jobLink, positionName, companyName, hrActiveFilter, progressCtx) {
    ...
    var card = await this.scrollAndFindCard(jobLink, positionName, companyName, fastMode);
...
async function handleBatchExtract(msg) {
  ...
  for (var i = 0; i < queue.length; i++) {
    ...
    var itemTimeout = msg.fastMode ? 10000 : 15000;
    try {
      ...
      var clickResult = await Promise.race([
        JobClicker.clickImmediateChat(item.jobLink, item.positionName, item.companyName, msg.hrActiveFilter,
          { index: i, jobId: item.jobId, jobName: item.positionName, fastMode: !!msg.fastMode }),
