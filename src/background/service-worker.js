...
function getSendSpeedProfile() {
  ...
  var fast = !!(state.testMode || n <= 5);
  return {
    fast: fast,
    postExtractMs: fast ? 600 : (CONFIG.POST_EXTRACT_DELAY_MS || 3000),
    navSettleMs: fast ? 700 : 2000,
    workerCount: fast ? 1 : Math.min(CONFIG.MAX_SEND_WORKERS || 3, n),
    repairSettleMs: fast ? 800 : 3000,
  };
}
...
function getStage1QueueSlice() {
  return state.sendQueueV6.filter(function (item) {
    ...
    if (!state._stage1SearchUrlFilter) return true;
    var u = item.searchUrl || getJobsPageUrl();
    return u === state._stage1SearchUrlFilter;
  });
}
...
function collectPendingSearchUrls() {
  ...
  state.sendQueueV6.forEach(function (item) {
    if (item.hrName || item.extractError) return;
    var u = item.searchUrl || getJobsPageUrl();
...
function markStage1PageFailures(searchUrl, reason) {
  state.sendQueueV6.forEach(function (item) {
    if (item.hrName || item.searchUrl !== searchUrl) return;
...
function getStage1PendingItems() {
  return state.sendQueueV6.filter(function (item) {
    return item && !item.hrName && !item.extractError;
  });
}
...
async function runStage1SingleJob(item, idx, total) {
  ...
  try {
    await new Promise(function (resolve) {
      ...
      sendTabMessageWithBFCacheRetry(tabId, {
        ...
        hrActiveFilter: state.hrActiveFilter || '不限',
        fastMode: getSendSpeedProfile().fast,
  ...
  var extracted = state.sendQueueV6.filter(function (i) { return i.hrName; }).length;
  chrome.runtime.sendMessage({
    type: MSG.SEND_PROGRESS,
    sent: extracted,
    total: state.sendQueueV6.length,
    status: '正在提取HR信息 (' + idx + '/' + total + ')',
  }).catch(function () {});
}
...
async function releaseCollectTab() {
  if (_collectTabId && _collectTabOwned) {
...
async function collectOnTab(cityCode, params) {
  ...
  const tabId = await acquireCollectTab(url);
...
async function ensureGreetingEnabled(searchTabId) {
  return ensureGreetingDisabledForSend(searchTabId);
}
...
async function startSendV6(jobIds) {
  ...
  // pre-flight：关闭 BOSS「自动打招呼」，避免平台模板语与 AI 招呼语双发
  ...
  console.log('[即投] v6 stage1 开始，队列长度:', state.sendQueueV6.length);
  ...
  if (_extractFailed.length) {
    console.log('[即投] startSendV6: 记录', _extractFailed.length, '个提取失败岗位');
    pushState();
  }
  ...
  if (_skippedAlready.length) {
    console.log('[即投] startSendV6: 跳过', _skippedAlready.length, '个已沟通过的岗位');
    pushState();
  }
...
async function runStage1() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var handler = function(msg, sender) {
      if (msg.type === MSG.EXTRACT_COMPLETE && sender.tab && sender.tab.id === state.searchTabId) {
        ...
        if (msg.stage === 'beforeClick') {
          ...
          console.log('[即投] runStage1: 提取进度', msg.extracted + '/' + msg.total);
          chrome.runtime.sendMessage({
            type: MSG.SEND_PROGRESS,
            sent: msg.extracted,
            total: msg.total,
    ...
    var doSend = function(retryCount) {
      ...
      var _fast = getSendSpeedProfile().fast;
      chrome.tabs.sendMessage(state.searchTabId, {
        type: MSG.DO_BATCH_EXTRACT,
        queue: queueSlice,
        hrActiveFilter: state.hrActiveFilter || '不限',
