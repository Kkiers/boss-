...
(function () {
  var _state = {
    ...
    workAreasByCity: {}, // {cityCode:[区名]}，每城独立工作区域；缺省视作 ['不限']
    jobTypes: ['不限'],
    salaryRanges: ['不限'],
    experience: ['不限'],
    education: ['不限'],
    ...
    groupExpanded: {},
    testMode: false,
    testJobsPerPosition: 1,
    sendMode: 'platform', // platform=快速（BOSS自动招呼）；custom=定制招呼语+简历图
    sendResumeImage: false, // 快速投递时是否同时发送简历图片；true=点「继续沟通」进聊天页发图；false=点「留在此页」
    workMode: 'search',   // search=搜索采集；browse=首页浏览投递
    browseScope: 'current', // current=当前页面；recommend=推荐标签轮询
    browseSessionLimit: 0,  // 0=不限
    browseSourceTabId: null,
    browseSourceTabUrl: '',
    browsing: false,
  };
