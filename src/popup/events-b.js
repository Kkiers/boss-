...
window.initEventsB=function(){
  ...
  function persistSendMode(mode) {
    Store.set('sendMode', mode);
    try {
      if (typeof STORAGE_KEYS !== 'undefined' && chrome.storage && chrome.storage.local) {
        chrome.storage.local.get(STORAGE_KEYS.UI.FILTER_STATE, function (r) {
          var fs = r[STORAGE_KEYS.UI.FILTER_STATE] || {};
          fs.sendMode = mode;
          var save = {};
          save[STORAGE_KEYS.UI.FILTER_STATE] = fs;
  ...
  window.syncSendModeUI = function () {
    var mode = Store.get('sendMode') || 'platform';
    var chips = document.querySelectorAll('[data-send-mode]');
    for (var i = 0; i < chips.length; i++) {
      chips[i].classList.toggle('active', chips[i].getAttribute('data-send-mode') === mode);
    }
    var hint = document.getElementById('sendModeHint');
    if (hint) {
      hint.textContent = mode === 'custom'
        ? '关闭 BOSS 自动招呼，按职位发送定制招呼语与简历图（较慢，适合重点岗位）'
        : '使用 BOSS 内置招呼语，搜索页批量建联，约 2-3 分钟完成';
    }
    var tipTitle = document.querySelector('#progressTip .tip-title');
    if (tipTitle) {
      tipTitle.textContent = mode === 'custom'
        ? '🚀 定制投递中 · 耗时较长，请耐心等待'
        : '🚀 快速投递中 · 约 2-3 分钟';
    }
    // 简历图片选项：仅在快速投递模式下显示
    var label = document.getElementById('sendResumeImageLabel');
    var cb = document.getElementById('sendResumeImageCheckbox');
    if (label) label.style.display = mode === 'platform' ? 'flex' : 'none';
    if (cb) cb.checked = !!Store.get('sendResumeImage');
  };

  var sendModeBar = document.getElementById('sendModeBar');
  if (sendModeBar) {
    sendModeBar.addEventListener('click', function (e) {
      if (Store.get('sending')) return;
      var chip = e.target.closest('[data-send-mode]');
      if (!chip) return;
      var mode = chip.getAttribute('data-send-mode');
      if (!mode || mode === (Store.get('sendMode') || 'platform')) return;
      persistSendMode(mode);
      window.syncSendModeUI();
    });
    window.syncSendModeUI();
  }

  // 简历图片复选框：仅在快速投递模式下生效
  var sendResumeImageCb = document.getElementById('sendResumeImageCheckbox');
  if (sendResumeImageCb) {
    sendResumeImageCb.addEventListener('change', function () {
      Store.set('sendResumeImage', !!sendResumeImageCb.checked);
        ...
        if(gact.dataset.gact==='rewrite'){
          ...
          chrome.runtime.sendMessage({type:MSG.REGENERATE_GREETING,category:g.position,jdSamples:jdSamples},function(resp){
            if(resp&&resp.success&&resp.greeting){
              ...
              alert(resp.error||'招呼语生成失败，请确认已上传清晰简历');
            }
  ...
  // ── 停止采集 ──
  if(E.btnStopCollect){
    E.btnStopCollect.addEventListener('click',function(){
      if(!Store.get('collecting'))return;
      try{if(typeof DiagLogger!=='undefined')DiagLogger.userEvent('popup','用户点击「停止采集」按钮')}catch(_){}
      E.btnStopCollect.textContent='正在停止...';
      E.btnStopCollect.disabled=true;
      E.progressText.textContent='正在停止采集...';
      E.progressSub.textContent='请稍候';
      Store.set('collecting',false);
      Store.set('awaitingCollect',false);
      try{
        chrome.runtime.sendMessage({type:MSG.STOP_COLLECT},function(resp){
          if(chrome.runtime.lastError){
            E.progressText.textContent='采集已停止';
            E.progressSub.textContent='扩展后台未响应，已重置界面';
            if(typeof window.resetStopCollectBtn==='function')window.resetStopCollectBtn();
            window.toSettings&&window.toSettings();
            return;
          }
        });
      }catch(ex){
      ...
      setTimeout(function(){
        if(E.progressText&&E.progressText.textContent==='正在停止采集...'){
          E.progressText.textContent='采集已停止';
          E.progressSub.textContent='';
          if(typeof window.resetStopCollectBtn==='function')window.resetStopCollectBtn();
        }
  ...
  function startSendAfterGate(jobIds){
    ...
    try{
      chrome.runtime.sendMessage({
        ...
        sendMode: Store.get('sendMode') || 'platform',
        sendResumeImage: !!Store.get('sendResumeImage')
      }, function(resp){
        if(chrome.runtime.lastError||!resp||!resp.success){
          ...
          showNoQuotaBuy(false);
          E.progressText.textContent=(Store.get('sendMode')==='custom')?'正在定制投递...':'正在快速投递...';
          E.progressSub.classList.remove('hidden');
          E.progressSub.textContent='共 '+jobIds.length+' 个岗位 · '+(Store.get('sendMode')==='custom'?'定制招呼语+简历图':'BOSS 平台招呼语');
        }
      });
    }catch(ex){
      Store.set('sending',false);
  ...
  // ── 无免费额度内联购买入口：显隐 + 复用 account.js 的购买跳转（window.JitouBuy）──
  function showNoQuotaBuy(show){
    var box=document.getElementById('noQuotaBuy');
    if(!box)return;
    if(show)box.classList.remove('hidden');
    else box.classList.add('hidden');
  }
  (function wireNoQuotaBuy(){
    var b7=document.getElementById('noQuotaBuy7');
    var b30=document.getElementById('noQuotaBuy30');
    if(b7)b7.addEventListener('click',function(){if(window.JitouBuy&&window.JitouBuy.buy7)window.JitouBuy.buy7();});
    if(b30)b30.addEventListener('click',function(){if(window.JitouBuy&&window.JitouBuy.buy30)window.JitouBuy.buy30();});
  })();
};
